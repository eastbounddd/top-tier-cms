...
export default async function Home() {
  ...
  try {
    ...
    const { data } = await supabase.from("articles").select("id,title,slug,excerpt,cover_image_url,category,published_at,is_top_story,school").eq("status","published").order("published_at",{ascending:false}).limit(24);
    articles = data ?? [];
  } catch {}
